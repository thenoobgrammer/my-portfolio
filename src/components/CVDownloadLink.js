...
const CV_PDFDownloadLink = () => {
    ...
    const CV_PDF = () => {
        return (
            <Document pageLayout="oneColumn">
                <Page size="A4" style={{ fontFamily: 'Roboto', padding: 20 }}>
    ...
    return (
        <PDFDownloadLink document={<CV_PDF />} fileName={t('CV_update_to_date')}>
            {({ loading }) => (loading ? <Spinner /> : <div className='flex items-center gap-x-4 hover:text-primary/60'>{t('Download')}<AiOutlineCloudDownload size={25} /></div>)}
        </PDFDownloadLink>
    )
}
